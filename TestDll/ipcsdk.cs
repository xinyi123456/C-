...
namespace TestDll
{
    ...
    public partial class ipcsdk
    {
       ...
       * 1.用户自定义参数，用来区分不同的sdk使用者，类似于线程入口函数的参数
         2.payload类型，取值为上面所定义的宏
        3.时间戳
        4.收到的数据地址
        5.收到的数据长度
      */
        public delegate void ICE_IPCSDK_OnStream(System.IntPtr pvParam, byte u8PayloadType, uint u32Timestamp, System.IntPtr pvData, int s32Len);

        /**
       *  @brief  设置网络流回调
       *  @param  [IN] hSDK       连接相机时返回的sdk句柄
       *  @param  [IN] pfOnFrame  网络流回调接口，当网络上有数据时，sdk会回调该接口，（详见说明6）
       *  @param  [IN] pvParam    回调函数中的参数，能区分开不同的使用者即可
       *  @return void
       */
        [System.Runtime.InteropServices.DllImportAttribute("ice_ipcsdk.dll", EntryPoint = "ICE_IPCSDK_SetDataCallback", CallingConvention = CallingConvention.Cdecl)]
        public static extern void ICE_IPCSDK_SetDataCallback(System.IntPtr hSDK, ICE_IPCSDK_OnStream pfOnStream, System.IntPtr pvParam);
    }
}
