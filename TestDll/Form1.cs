...
namespace TestDll
{
    public partial class Form1 : Form
    {
            ...
            Int32 s32Width, Int32 s32Height, Int32 i)
        {
            ...
            if (0 == (frame_count[i] % 30))
            {
                ...
                try
                {
                    ...
                    //Bitmap bitmap = new Bitmap(s32Width, s32Height, PixelFormat.Format24bppRgb);
                    //BitmapData bmData = bitmap.LockBits(new System.Drawing.Rectangle(0, 0, bitmap.Width, bitmap.Height), ImageLockMode.ReadWrite, bitmap.PixelFormat);
                    //IntPtr pNative = bmData.Scan0;
                    //Marshal.Copy(rgb24, 0, pNative, s32Width * s32Height * 3);
                    //bitmap.UnlockBits(bmData);

                    //this.pictureBox3.BackgroundImage = bitmap;
        ...
        public byte[] lastFrame = new byte[0];
        public void SDK_OnStream(System.IntPtr pvParam, byte u8PayloadType, uint u32Timestamp, System.IntPtr pvData, int s32Len)
        {
            int index = (int)pvParam;
            if (m_bExit || bClose[index])
                return;

            byte[] buf = new byte[s32Len];
            Marshal.Copy(pvData, buf, 0, s32Len);

            foreach (var item in buf)
            {
                DataFlow.Enqueue(item);
            }
        }

        public void onDrawDataFlow()
        {
            CheckForIllegalCrossThreadCalls = false;////避免跨线程的问题

            bool isEnd = false;
            int bufferLen = 0x2000;
            //码流段
            byte[] buf = new byte[bufferLen];
            while (!isEnd)
            {
                //获取一段码流,积累一定缓存量再解
                if (DataFlow.Count >= bufferLen || isStop == 1)
                {
                    byte tempByte;
                    int j = 0;
                    for (int i = 0; i < bufferLen; i++)
                    {
                        if (DataFlow.TryDequeue(out tempByte))
                            buf[j++] = tempByte;
                        else
                        {
                            break;
                    ...
                    int result = 0;
                    result = H264Dec.Hi264DecFrame(_decHandle, pData, (UInt32)j, 0, ref _decodeFrame, (UInt32)isStop);
                    while (H264Dec.HI_H264DEC_NEED_MORE_BITS != result)
                    {
                        if (H264Dec.HI_H264DEC_NO_PICTURE == result)
                        {
                            isEnd = true;
                            break;
                        }
                        if (H264Dec.HI_H264DEC_OK == result)/* 输出一帧图像 */
                        {
                            //获取yuv
                            ...
                            byte[] rgb24 = new byte[_decodeFrame.uWidth * _decodeFrame.uHeight * 3];

                            util.Convert((int)_decodeFrame.uWidth, (int)_decodeFrame.uHeight, y, u, v, ref rgb24);

                            Bitmap bitmap = new Bitmap((int)_decodeFrame.uWidth, (int)_decodeFrame.uHeight, PixelFormat.Format24bppRgb);
                            BitmapData bmData = bitmap.LockBits(new System.Drawing.Rectangle(0, 0, bitmap.Width, bitmap.Height), ImageLockMode.ReadWrite, bitmap.PixelFormat);
                            IntPtr pNative = bmData.Scan0;
                            Marshal.Copy(rgb24, 0, pNative, (int)_decodeFrame.uWidth * (int)_decodeFrame.uHeight * 3);
                            bitmap.UnlockBits(bmData);
        ...
        private ipcsdk.ICE_IPCSDK_OnStream onStream;
        ...
        private void buttonConnect_Click(object sender, EventArgs e)
        {
            ...
            else if (radioButton_Frame.Checked == true) //选择连接方法为：图像解码
            {
                ...
                ipcsdk.ICE_IPCSDK_SetDataCallback(pUid[0], onStream, new IntPtr(0));
            }
        ...
        /// 解码器输出图像信息
        /// </summary>
        public H264Dec.hiH264_DEC_FRAME_S _decodeFrame;
        /// <summary>
        /// 解码器句柄
        /// </summary>
        public IntPtr _decHandle;
        public int H264result = 999;
        /// <summary>
        ///网络数据流
        /// </summary>
        public ConcurrentQueue<byte> DataFlow = new ConcurrentQueue<byte>();

        public delegate void DrawDataFlow();
        public DrawDataFlow drawDataFlows;
        public int isStop = 0;
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            decAttr.uPicWidthInMB = 1280;
            decAttr.uPicHeightInMB = 760;
            /* 解码器最大参考帧数: 16 */
            decAttr.uBufNum = 16;
            ...
            onStream = new ipcsdk.ICE_IPCSDK_OnStream(SDK_OnStream);

            //drawDataFlows = new DrawDataFlow(onDrawDataFlow);
            //this.BeginInvoke(drawDataFlows);

            Thread drawDataFlow = new Thread(new ThreadStart(onDrawDataFlow));
            drawDataFlow.Start();
